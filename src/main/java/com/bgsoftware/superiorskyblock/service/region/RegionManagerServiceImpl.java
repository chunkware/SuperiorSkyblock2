...
public class RegionManagerServiceImpl implements RegionManagerService, IService {
	...
	public InteractionResult handleBlockBreak(SuperiorPlayer superiorPlayer, Block block) {
		...
		try (ObjectsPools.Wrapper<Location> wrapper = ObjectsPools.LOCATION.obtain()) {
			...
			IslandPrivilege islandPrivilege = blockType == Materials.SPAWNER.toBukkitType() ? IslandPrivileges.SPAWNER_BREAK
	...
	public InteractionResult handleBlockInteract(SuperiorPlayer superiorPlayer, Block block, Action action,
		...
		try (ObjectsPools.Wrapper<Location> wrapper = ObjectsPools.LOCATION.obtain()) {
			...
			if (usedItem != null && Materials.isMinecart(usedItem.getType()) ? Materials.isRail(blockType)
					: Materials.isBoat(blockType)) {
			...
			} else if (usedItem != null && blockType == VAULT
					&& (usedItem.getType() == TRIAL_KEY || usedItem.getType() == OMINOUS_TRIAL_KEY)) {
				islandPrivilege = IslandPrivileges.USE;
				...
				islandPrivilege = Materials.BONE_MEAL.toBukkitItem().isSimilar(usedItem) ? IslandPrivileges.FERTILIZE
						: IslandPrivileges.FARM_TRAMPING;
	...
	@Override
	public InteractionResult handleEntityInteract(SuperiorPlayer superiorPlayer, Entity entity,
			@Nullable ItemStack usedItem) {
		Preconditions.checkNotNull(superiorPlayer, "superiorPlayer cannot be null");
		...
		try (ObjectsPools.Wrapper<Location> wrapper = ObjectsPools.LOCATION.obtain()) {
			...
			if (entity instanceof ArmorStand) {
			...
			} else if (usedItemType == Material.WATER_BUCKET && entityType == AXOLOTL_TYPE
					&& ServerVersion.isAtLeast(ServerVersion.v1_17)) {
				...
				islandPrivilege = BukkitEntities.isTameable(entity) ? IslandPrivileges.TAMED_ANIMAL_DAMAGE
						: IslandPrivileges.ANIMAL_DAMAGE;
			} else {
				return InteractionResult.SUCCESS;
			}
			...
			BukkitExecutor.sync(() -> {
				...
				if (player != null && player.isOnline()) {
					...
					if (openInventory != null
							&& (openInventory.getType() == InventoryType.MERCHANT || openInventory.getType() == InventoryType.CHEST))
	...
	public InteractionResult handleEntityDamage(Entity damager, Entity entity) {
		...
		Optional<SuperiorPlayer> damagerSource = BukkitEntities.getPlayerSource(damager)
				.map(plugin.getPlayers()::getSuperiorPlayer);
	...
	public InteractionResult handlePlayerConsumeChorusFruit(SuperiorPlayer superiorPlayer, Location location) {
		...
		if (IslandPrivileges.CHORUS_FRUIT == null) {
			// Chorus Fruit privilege does not exist, we will just return SUCCESS in this
			// case.
			return InteractionResult.SUCCESS;
		}
	...
	public InteractionResult handlePlayerUseWindCharge(SuperiorPlayer superiorPlayer, Location location) {
		...
		if (IslandPrivileges.WIND_CHARGE == null) {
			// Wind Charge privilege does not exist, we will just return SUCCESS in this
			// case.
			return InteractionResult.SUCCESS;
		}
	...
	public MoveResult handlePlayerMove(SuperiorPlayer superiorPlayer, Location from, Location to) {
		...
		if (from.getBlockX() != to.getBlockX() || from.getBlockZ() != to.getBlockZ()) {
			...
			if (toIsland != null) {
				moveResult = handlePlayerEnterIslandInternal(superiorPlayer, toIsland, from, to,
			...
			if (fromIsland != null) {
				moveResult = handlePlayerLeaveIslandInternal(superiorPlayer, fromIsland, from, to,
		...
		if (from.getBlockY() != to.getBlockY() && to.getBlockY() <= plugin.getNMSWorld().getMinHeight(to.getWorld()) - 5) {
			...
			if (fromIsland == null
					|| (fromIsland.isVisitor(superiorPlayer, false) ? !plugin.getSettings().getVoidTeleport().isVisitors()
							: !plugin.getSettings().getVoidTeleport().isMembers()))
	...
	public MoveResult handlePlayerTeleport(SuperiorPlayer superiorPlayer, Location from, Location to) {
		...
		Island toIsland = plugin.getGrid().getIslandAt(to);
		if (toIsland != null) {
			MoveResult enterMove = handlePlayerEnterIslandInternal(superiorPlayer, toIsland, from, to,
		...
		if (fromIsland != null) {
			return handlePlayerLeaveIslandInternal(superiorPlayer, fromIsland, from, to,
	...
	@Override
	public MoveResult handlePlayerTeleportByPortal(SuperiorPlayer superiorPlayer, Location portalLocation,
	...
	public MoveResult handlePlayerJoin(SuperiorPlayer superiorPlayer, Location location) {
		...
		return island == null ? MoveResult.SUCCESS
				: handlePlayerEnterIslandInternal(superiorPlayer, island, null, location,
	...
	public MoveResult handlePlayerQuit(SuperiorPlayer superiorPlayer, Location location) {
		...
		return handlePlayerLeaveIslandInternal(superiorPlayer, island, location, null,
	...
	private MoveResult handlePlayerEnterIslandInternal(SuperiorPlayer superiorPlayer, Island toIsland,
			@Nullable Location from, Location to, IslandEnterEvent.EnterCause enterCause) {
		...
		if (toIsland.isBanned(superiorPlayer) && !superiorPlayer.hasBypassModeEnabled()
			...
			PluginEventsFactory.callIslandRestrictMoveEvent(toIsland, superiorPlayer,
					IslandRestrictMoveEvent.RestrictReason.BANNED_FROM_ISLAND);
		...
		if (toIsland.isLocked() && !toIsland.hasPermission(superiorPlayer, IslandPrivileges.CLOSE_BYPASS)) {
			PluginEventsFactory.callIslandRestrictMoveEvent(toIsland, superiorPlayer,
					IslandRestrictMoveEvent.RestrictReason.LOCKED_ISLAND);
		...
		if (toInsideRange && (!equalIslands || !fromInsideRange)
				&& !PluginEventsFactory.callIslandEnterProtectedEvent(toIsland, superiorPlayer, enterCause)) {
			PluginEventsFactory.callIslandRestrictMoveEvent(toIsland, superiorPlayer,
					IslandRestrictMoveEvent.RestrictReason.ENTER_PROTECTED_EVENT_CANCELLED);
		...
		if (!PluginEventsFactory.callIslandEnterEvent(toIsland, superiorPlayer, enterCause)) {
			PluginEventsFactory.callIslandRestrictMoveEvent(toIsland, superiorPlayer,
					IslandRestrictMoveEvent.RestrictReason.ENTER_EVENT_CANCELLED);
			return MoveResult.ENTER_EVENT_CANCELLED;
		}
		...
		if (!toIsland.isMember(superiorPlayer) && toIsland.hasSettingsEnabled(IslandFlags.PVP)) {
			...
			String titleMsg = Message.ENTER_PVP_ISLAND_TITLE.getMessage(locale);
			String subtitleMsg = Message.ENTER_PVP_ISLAND_SUBTITLE.getMessage(locale);

			if ((titleMsg != null && !titleMsg.isEmpty()) || (subtitleMsg != null && !subtitleMsg.isEmpty())) {
				Player player = superiorPlayer.asPlayer();
				if (player instanceof Audience) {
					Title.Times times = Title.Times.of(Duration.ofMillis(500), Duration.ofMillis(3000),
							Duration.ofMillis(1000));
					Title title = Title.title(
							Component.text(titleMsg == null ? "" : titleMsg),
							Component.text(subtitleMsg == null ? "" : subtitleMsg),
							times);
	...
	private MoveResult handlePlayerLeaveIslandInternal(SuperiorPlayer superiorPlayer, Island fromIsland, Location from,
		...
		if (plugin.getSettings().isStopLeaving() && fromInsideRange && !toInsideRange
			...
			PluginEventsFactory.callIslandRestrictMoveEvent(fromIsland, superiorPlayer,
					IslandRestrictMoveEvent.RestrictReason.LEAVE_ISLAND_TO_OUTSIDE);
		...
		if (fromInsideRange && (!equalIslands || !toInsideRange)) {
			if (!PluginEventsFactory.callIslandLeaveProtectedEvent(fromIsland, superiorPlayer, leaveCause, to)) {
				PluginEventsFactory.callIslandRestrictMoveEvent(fromIsland, superiorPlayer,
						IslandRestrictMoveEvent.RestrictReason.LEAVE_PROTECTED_EVENT_CANCELLED);
				return MoveResult.ENTER_EVENT_CANCELLED;
			}
		}
		...
		if (!PluginEventsFactory.callIslandLeaveEvent(fromIsland, superiorPlayer, leaveCause, to)) {
			PluginEventsFactory.callIslandRestrictMoveEvent(fromIsland, superiorPlayer,
					IslandRestrictMoveEvent.RestrictReason.LEAVE_EVENT_CANCELLED);
			return MoveResult.ENTER_EVENT_CANCELLED;
		}
		...
		if (player != null) {
			...
			if (superiorPlayer.hasIslandFlyEnabled() && (toIsland == null || toIsland.isSpawn())
					&& !superiorPlayer.hasFlyGamemode()) {
